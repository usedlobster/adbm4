        ...
        private function getLexer()
        {
            try
            {
                if ( $this->_lexer === null )
                {
                    ...
                    //
                    $lexer->pattern = '/"[^"]*"|'.'\'[^\']*\'|'.'[A-Za-z_][A-Za-z_0-9]*(\.[A-Za-z_0-9]*|'.'\([^\)]*\))*|'.'\d*\.?\d+|'.'\(|\)|'.',|'.join('|' , $escapeAble).'|[^\s]/u';
                    $this->_lexer = $lexer;
                }
            }
            catch ( \Exception $ex )
                ...
                while ( $i < $tokenCount )
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            elseif ( in_array($token , $lex->unary) && ($prev === null || in_array($prev , ['par_open' , ',' , 'var' , 'op'])) )
                            {
                                // is a unary operator
