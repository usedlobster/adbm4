<?php
    ...
    class SQL_PDO
    {
        ...
        public function __construct()
        {
            ...
            $this->_dsn      = null ;
            $this->_db       = null ;
            $this->_dbmaster = $_ENV['DB_MASTER'] ?? null ;
            $this->_pdo      = null;
        }
        ...
        private function makePDO( )
        {
            try
            {
                if ( !$this->_dsn )
                {
                    $db  = $this->_db ?? $this->_dbmaster ?? null;
                    $dsn = $_ENV[ 'DB_DSN' ] ?? null;
                    if ( !$dsn )
                        throw new \RuntimeException( 'DB_DSN is not defined' );
                    if ( $db )
                        $dsn = str_replace( '<DB>' , $db , $dsn );
                ...
                if ( !$this->_pdo )
                {
                    try
                    {
                        $this->_pdo = new PDO( $this->_dsn , $_ENV[ 'DB_USER' ] ?? '' , $_ENV[ 'DB_PASS' ] , [
        ...
        private function _exec($qry , $args) : bool
        {
            ...
            try
            {
                ...
                $qry = str_replace(['<DB>','<DBM>'], [ $this->_db , $this->_dbmaster ] , $qry ) ;
        ...
        public function LockExec( $qry , $args =[] , $lock= '') {
            ...
            try
            {
                ...
                if ( !empty( $lock ) )
                {
                    ...
                    {
                        $pdo = $this->_pdo ? : $this->makePDO();
                        $pdo->exec( 'LOCK TABLES ' . str_replace(['<DB>','<DBM>'], [$this->_db , $this->_dbmaster ] , $lock ) );
